...
export default class MTGManaIcons extends React.Component {
    ...
    createFullMana = (symbol) => {
        ...
        if (symbol === 'U') {
            bgColor = this.U_Color
            symbolSVG = this.U_Symbol
        } else if (symbol === 'B') {
            bgColor = this.B_Color
            symbolSVG = this.B_Symbol
        } else if (symbol === 'G') {
            bgColor = this.G_Color
            symbolSVG = this.G_Symbol
        } else if (symbol === 'W') {
            bgColor = this.W_Color
            symbolSVG = this.W_Symbol
        } else if (symbol === 'R') {
            bgColor = this.R_Color
            symbolSVG = this.R_Symbol
        } else {
            bgColor = '#ffffff'
            symbolSVG = <g>
                            <circle cx="300" cy="300" r="300" fill={this.B_Color}/>
                            <text x="200" y="400" style={{fontSize:'400px', fill: 'black'}}>{symbol}</text>
                        </g>
        }
    ...
    createHalfManaLeft = (symbol) => {
        ...
        if (symbol === 'U') {
            ...
            bgColor = '#ffffff'
            symbolSVG = <text x="200" y="400" style={{fontSize:'400px', fill: 'black'}}>{symbol}</text>
        }

        return (
            <g>
                <path d="M0,300 a1,1 0 0,0 600,0" fill={bgColor} transform="rotate(135 300 300)" />
    ...
    createHalfManaRight = (symbol) => {
        ...
        return (
            <g>
                <path d="M0,300 a1,1 0 0,0 600,0" fill={bgColor} transform="rotate(-45 300 300)" />
                <g transform="scale(0.5) translate(500, 500)">
                    {symbolSVG}
                </g>
            </g>
        )
    ...
    createManaIcons = (manaString) => {
        if (!manaString) {
            return []
        }

        let icons = []
        let manaRegexp = /\{([A-Z0-9/]+)\}/gi
        let match = manaRegexp.exec(manaString)
        ...
        while (match !== null) {
            ...
            match = manaRegexp.exec(manaString)
        }

        return icons
    }

    prepareText = (targetText) => {
        // targetText = targetText.replace(/\n/g, <br/>)
        let parts = []
        let lastIndex = 0
        let manaRegexp = /\{([A-Z0-9/]+)\}/gi
        let match = manaRegexp.exec(targetText)
        ...
        while (match !== null) {
            ...
            match = manaRegexp.exec(targetText)
        }
    ...
    render = () => {
        ...
        if (this.props.inlineText) {
            let parts = this.prepareText(this.props.inlineText)
            return (
                <div className="mtg-mana-text-container">
                    {parts}
                </div>
            )
        }

        let icons = this.createManaIcons(this.props.manaString)

        return (
            <div className="mtg-mana-icons-container">
                {icons}
            </div>
        )
